...
for d in np.unique(train_day):
    ...
    sg[sg < 1e-8] = 1.0
    day_stats[d] = (mu, sg)
print(f'day_stats keys={len(day_stats)}', flush=True)
...
lgb_og_raw = train_lgb(X_tr_og, X_te_og, y_wins, groups5, 'lgb_og')
lgb_og = finalize(lgb_og_raw)
lgb_og_ic = daywise_ic(lgb_og, oracle_df, test_ids, test_day)
print(f'lgb_og: IC={lgb_og_ic:+.6f} std={lgb_og.std():.3e}', flush=True)
...
print('\n=== BLENDS (per-day feat norm + global shift only) ===', flush=True)
...
for w_lgb in [0.90, 0.85, 0.80, 0.75, 0.70]:
    ...
    submissions[f'lgb_all_{int(round(w_lgb*100))}_grin_all_{int(round(w_g*100))}'] = \
        blend([(w_lgb, lgb_all_raw), (w_g, grin_all)])
