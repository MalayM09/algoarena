...
print('Global normalization (training mean/std, row-wise, compliant)...', flush=True)
tr_raw = train[all_feat].fillna(0).values.astype(np.float32)
te_raw = test[all_feat].fillna(0).values.astype(np.float32)

global_mean = tr_raw.mean(0)
global_std = tr_raw.std(0)
global_std[global_std < 1e-8] = 1.0

X_tr = np.clip((tr_raw.astype(np.float64) - global_mean) / global_std,
               -CLIP_Z, CLIP_Z).astype(np.float32)
X_te = np.clip((te_raw.astype(np.float64) - global_mean) / global_std,
               -CLIP_Z, CLIP_Z).astype(np.float32)
del tr_raw, te_raw
gc.collect()
...
orig_gold_set = {k for k, v in spearman_icir.items()
                 if v['abs_icir'] >= ICIR_GOLD and v['ic_pos'] in (0.0, 1.0)}
print(f'orig_gold={len(orig_gold_set)}', flush=True)
...
gold_all = {k: v for k, v in icir_all.items()
            if v['abs_icir'] >= ICIR_GOLD and v['ic_pos'] in (0.0, 1.0)}
gold_orig_feats = sorted([k for k in gold_all if k in all_feat],
...
orig_gold_idx = [all_feat.index(f) for f in gold_orig_feats]
...
print('\n=== GRINOLD (row-wise dot product) ===', flush=True)
ic_w_og = np.array([all_gold_ic[f] for f in gold_orig_feats], dtype=np.float64)
...
print(f'grin_orig: IC={grin_og_ic:+.6f} std={grin_og.std():.3e}', flush=True)
...
print(f'grin_all:  IC={grin_all_ic:+.6f} std={grin_all.std():.3e}', flush=True)
...
lgb_og_raw = train_lgb(X_tr_og, X_te_og, y_wins, groups5, 'lgb_orig')
lgb_og = finalize(lgb_og_raw)
lgb_og_ic = daywise_ic(lgb_og, oracle_df, test_ids, test_day)
print(f'lgb_orig: IC={lgb_og_ic:+.6f} std={lgb_og.std():.3e}', flush=True)

lgb_all_raw = train_lgb(X_tr_all, X_te_all, y_wins, groups5, 'lgb_all')
lgb_all = finalize(lgb_all_raw)
lgb_all_ic = daywise_ic(lgb_all, oracle_df, test_ids, test_day)
print(f'lgb_all:  IC={lgb_all_ic:+.6f} std={lgb_all.std():.3e}', flush=True)
...
def train_ridge(X_train, X_test, y, groups, alphas, label):
    ...
    for alpha in alphas:
        ...
        print(f'  ridge {label} alpha={alpha}: IC={ic:+.6f}', flush=True)
        if ic > best_ic:
            best_ic = ic
            best_pred = te_s
            best_alpha = alpha
    print(f'  best ridge {label}: alpha={best_alpha} IC={best_ic:+.6f}', flush=True)
    return best_pred, best_ic

alphas = [100, 500, 1000, 5000, 10000]
ridge_og, ridge_og_ic = train_ridge(X_tr_og, X_te_og, y_wins, groups5, alphas, 'orig')
ridge_all, ridge_all_ic = train_ridge(X_tr_all, X_te_all, y_wins, groups5, alphas, 'all')

print('\n=== BLENDS (all compliant) ===', flush=True)
...
submissions['solo_lgb_orig'] = lgb_og
submissions['solo_lgb_all'] = lgb_all
submissions['solo_grin_orig'] = grin_og
submissions['solo_grin_all'] = grin_all
submissions['solo_ridge_orig'] = ridge_og
submissions['solo_ridge_all'] = ridge_all

for w_lgb in [0.90, 0.85, 0.80, 0.75, 0.70, 0.65, 0.60]:
    w_g = 1.0 - w_lgb
    submissions[f'lgb_og_{int(round(w_lgb*100))}_grin_all_{int(round(w_g*100))}'] = \
        blend([(w_lgb, lgb_og_raw), (w_g, grin_all)])
    submissions[f'lgb_og_{int(round(w_lgb*100))}_grin_og_{int(round(w_g*100))}'] = \
        blend([(w_lgb, lgb_og_raw), (w_g, grin_og)])

for w_lgb, w_r, w_g in [(0.6, 0.2, 0.2), (0.5, 0.2, 0.3), (0.5, 0.3, 0.2),
                        (0.7, 0.15, 0.15), (0.4, 0.3, 0.3)]:
    name = f'lgb{int(w_lgb*100)}_ridgeog{int(w_r*100)}_grinall{int(w_g*100)}'
    submissions[name] = blend([(w_lgb, lgb_og_raw), (w_r, ridge_og), (w_g, grin_all)])
